...
SELECT user_no, 
       COALESCE(CONCAT(user_mobile1, user_mobile2), 'None') AS contact
  FROM tbl_user;
...
SELECT u.user_no AS 번호, user_id AS 아이디
  FROM tbl_user u
  LEFT JOIN tbl_buy b
...
SELECT prod_category AS 카테고리, COUNT(*) AS 구매횟수
  FROM tbl_product p INNER JOIN tbl_buy b on p.prod_code = b.prod_code
